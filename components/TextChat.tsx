...
import { AttachedFile, ScoreRecord } from '../types';
...
export const TextChat: React.FC<TextChatProps> = ({ courseContent, systemInstruction, apiKey, themeColor = 'blue' }) => {
  ...
  const sendMessage = async (overrideInput?: string, quizMsgIndex?: number, choiceLabel?: string) => {
      ...
      if (text === "Je préfère rester anonyme" || (looksLikeQuestion && !overrideInput) || (looksLikeQuickAction && !overrideInput)) {
        ...
        if (matches.length > 0) {
          ...
          setTimeout(() => {
            addMessageToSession(activeSessionId, {
              role: 'model',
              text: `Enchantée, **${profile.name}** ! Votre dossier est prêt (ID : ${profile.id}). En quoi puis-je vous éclairer aujourd'hui ?`,
              timestamp: new Date()
            });
            setIsLoading(false);
          }, 600);
    ...
    // Suite normale (Appel Backend Vercel)
    ...
    try {
      // ---------------------------------------------------------
      // MODIFICATION MAJEURE : APPEL API VERCEL AU LIEU DE GOOGLE
      // ---------------------------------------------------------
  ...
  const MessageRenderer = ({ text, msgIndex, role, selectedOption, selectedOptions }: { text: string, msgIndex: number, role: string, selectedOption?: string, selectedOptions?: string[] }) => {
    ...
    const hasChoiceMarker = text.includes('[CHOIX_SUITE]');
    const isExpanded = expandedResponses[msgIndex];

    let displayText = text;

    if (hasChoiceMarker) {
      const parts = text.split('[CHOIX_SUITE]');
      displayText = parts[0].trim();

      if (isExpanded && parts[1]) {
        const remainingText = parts[1].replace(/\[ \] 1\..*(\n|$)/, '').replace(/\[ \] 2\..*(\n|$)/, '').trim();
        if (remainingText) {
          displayText += '\n\n' + remainingText;
        }
      }
    ...
    const isMultiChoice = text.toLowerCase().includes("plusieurs r�ponses possibles");
    ...
    if (hasChoiceMarker) {
      const parts = text.split('[CHOIX_SUITE]');
      if (parts[1]) {
        const optionLines = parts[1].split('\n');
        optionLines.forEach(line => {
          const match = line.match(/\[ \] \d\.\s*(.*)/);
          if (match) {
            choiceOptions.push(match[1].trim());
          }
        });
      }
    }
    ...
    return (
      <div className="space-y-3">
        {segments.map((seg, i) => {
          if (seg.type === 'button') {
            ...
            const isNavButton = seg.content.toLowerCase().includes("oui") ||
              seg.content.toLowerCase().includes("pr�t") ||
        ...
        {hasChoiceMarker && !isExpanded && choiceOptions.length >= 2 && (
          <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-slate-50 dark:from-slate-800/30 dark:to-slate-900/30 rounded-xl border border-blue-100 dark:border-slate-700">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={() => toggleResponseExpansion(msgIndex)}
              ...
              <button
                ...
                className="flex-1 px-4 py-3 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg font-medium transition-all flex items-center justify-center gap-2"
              >
                <X size={18} />
                {choiceOptions[1] || "Non, c'est suffisant"}
              </button>
            </div>
          </div>
        )}

        {isExpanded && hasChoiceMarker && (
          <button
            onClick={() => toggleResponseExpansion(msgIndex)}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-50/30 dark:bg-slate-950">
        ...
        <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
          ...
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2 no-scrollbar px-1">
            ...
            <button onClick={() => sendMessage("G�n�re un QCM de 3 questions sur un ou plusieurs th�mes du cours que je vais t'indiquer")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-700 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 rounded-lg hover:bg-blue-100 transition-all whitespace-nowrap"><Search size={14} /> QCM</button>
            <button onClick={() => sendMessage("Propose-moi 3 affirmations Vrai/Faux sur un ou plusieurs th�mes du cours que je vais t'indiquer")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-rose-700 bg-rose-50 dark:bg-rose-900/30 border border-rose-200 rounded-lg hover:bg-rose-100 transition-all whitespace-nowrap"><BookOpen size={14} /> Vrai/Faux</button>
            <button onClick={() => sendMessage("Soumets-moi un petit cas pratique sur un ou plusieurs th�mes du cours que je vais t'indiquer.")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 rounded-lg hover:bg-emerald-100 transition-all whitespace-nowrap"><FileSignature size={14} /> Cas pratique</button>
            ...
            <button onClick={() => sendMessage("Propose-moi un sujet de dissertation et un plan d�taill� (I. II.) bas� sur un ou plusieurs th�mes du cours que je vais t'indiquer.")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-indigo-700 bg-indigo-50 dark:bg-indigo-900/30 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-100 transition-all whitespace-nowrap"><BookOpen size={14} /> Plan Dissertation</button>
            <button onClick={() => sendMessage("Listez les arr�ts li�s � des d�finitions, SVP.")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-purple-700 bg-purple-50 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800 rounded-lg hover:bg-purple-100 transition-all whitespace-nowrap"><BookOpen size={14} /> Arr�ts & d�finitions</button>
            <button onClick={() => sendMessage("Listez les arr�ts li�s � un ou plusieurs th�mes du cours que je vais vous indiquer.")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-cyan-700 bg-cyan-50 dark:bg-cyan-900/30 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800 rounded-lg hover:bg-cyan-100 transition-all whitespace-nowrap"><BookOpen size={14} /> Arr�ts & notions cl�s</button>
          </div>
          <div className="relative flex items-end gap-2 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-2xl border-2 border-slate-200 dark:border-slate-700">
            ...
            <textarea ref={textareaRef} rows={1} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), sendMessage())} placeholder={!currentProfile ? "Choisissez une option ou pr�sentez-vous..." : "Votre question juridique..."} className="flex-1 py-3 bg-transparent border-none outline-none resize-none text-slate-800 dark:text-white max-h-[200px]" />
